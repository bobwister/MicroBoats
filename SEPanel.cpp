...
SFActionBox::SFActionBox(sf::Font* font)
{
	for (int i = 0; i < NBVAL_ActionButtons; i++)
	{
		...
		m_boxes[i] = SEGameObject(sf::Vector2f(0, 0), sf::Vector2f(0, 0), filename, sf::Vector2f(INTERACTION_BUTTON_WIDTH, INTERACTION_BUTTON_HEIGHT), sf::Vector2f(INTERACTION_BUTTON_WIDTH / 2, INTERACTION_BUTTON_HEIGHT / 2));
...
{
	m_box = SEGameObject(sf::Vector2f(0, 0), sf::Vector2f(0, 0), INTERACTION_BUTTON_A_FILENAME, sf::Vector2f(INTERACTION_BUTTON_WIDTH, INTERACTION_BUTTON_HEIGHT), sf::Vector2f(INTERACTION_BUTTON_WIDTH / 2, INTERACTION_BUTTON_HEIGHT / 2));
	m_selected_index = -1;
}
...
//PANEL
SEPanel::SEPanel(sf::Vector2f size, SFPanelTypes panel_type)
{
	setSize(size);
	m_visible = true;
...
bool SEPanel::IsCursorCollidingWithRectangle(SEGameObject& cursor, RectangleShape& object)
{
	if (!cursor.m_visible)
	{
		return false;
...
void SEPanel::Update(sf::Time deltaTime)
{
	//see override function in specific types of SFPanels
}

void SEPanel::Draw(sf::RenderTexture& screen)
{
	if (m_visible)
	{
		screen.draw(*this);
